(ns provisdom.date.core
  (:require [provisdom.utility-belt
            ...
            [provisdom.date.specs :as date-specs])
...
(s/fdef leap-year?
        :args (s/cat :year ::date-specs/year)
        :ret boolean?)
...
(s/fdef days-per-month
        :args (s/cat :year ::date-specs/year :month ::date-specs/month)
        :ret ::date-specs/days-per-month)
...
(s/fdef passed-leap-days
        :args (s/cat :year ::date-specs/year :month ::date-specs/month
                     :d2 (s/? (s/cat :year ::date-specs/year :month ::date-specs/month)))
...
(s/fdef period
        :args (s/cat :ticks-or-interval (s/or :ticks ::date-specs/ticks :interval ::date-specs/interval))
        :ret ::date-specs/period)
...
(s/fdef as-ticks
        :args (s/cat :weeks ::date-specs/weeks :days (s/? ::date-specs/days) :ticks (s/? ::date-specs/ticks)
                     :more (s/? (s/keys* :opt-un [::date-specs/hr ::date-specs/mi ::date-specs/se ::date-specs/ms ::date-specs/us])))
        :ret ::date-specs/ticks)
...
(s/fdef duration
        :args (s/cat :years ::date-specs/years :months (s/? ::date-specs/months)
                     :days (s/? ::date-specs/days) :ticks (s/? ::date-specs/ticks)
                     :more (s/? (s/keys* :opt-un [::date-specs/wk ::date-specs/hr ::date-specs/mi ::date-specs/se ::date-specs/ms ::date-specs/us])))
        :ret ::date-specs/duration)
...
(s/fdef date                                                ;;also: A date must be entered as later than 7/8/1814 and earlier than 6/29/2325.
        :args (s/cat :year ::date-specs/year :month (s/? ::date-specs/month)
                     :day-number (s/? ::date-specs/day-number) :ticks (s/? ::date-specs/ticks)
                     :more (s/? (s/keys* :opt-un [::date-specs/hr ::date-specs/mi ::date-specs/se ::date-specs/ms ::date-specs/us])))
        :ret ::date-specs/date)
